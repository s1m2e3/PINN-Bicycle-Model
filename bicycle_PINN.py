...
class bicycle_PINN:
    ...
    def __init__(self,df,mod_type,nodes=10):
        ...
        self.train(self.act,self.control,states_train,states_test,"PIELM")
        self.XTFC(self.control.shape,self.states.shape,self.n_nodes)
        self.train(self.act,self.control,states_train,states_test,"XTFC")
    ...
    def train(self,activation_function,control,states,states_test,mode):
        ...
        if mode=="PIELM":
            ...
            U =tf.concat([xt,yt],axis=1)
            U = tf.concat([U,thetat],axis=1)
            B = tf.concat([C_,U],axis=0)
            self.betas = tf.linalg.lstsq(A,B,fast=False)
            after = datetime.datetime.now()
            print(after-now)
            new_preds = tf.matmul(F_,(self.betas))+Y[0,:]
            print(tf.reduce_sum(tf.math.pow(new_preds-Y,2)))
            return new_preds
