...
class PIELM:
    ...
    def predict_jacobian(self,betas):
        ...
        l_pred_x = self.y_train[:,0]-torch.matmul(self.get_h(self.x_train),betas[0:self.nodes])
        l_pred_y = self.y_train[:,1]-torch.matmul(self.get_h(self.x_train),betas[self.nodes:2*self.nodes])
        l_pred_theta = self.y_train[:,2]-torch.matmul(self.get_h(self.x_train),betas[self.nodes*2:3*self.nodes])
        l_pred_delta = self.y_train[:,3]-torch.matmul(self.get_h(self.x_train),betas[self.nodes*3:4*self.nodes])
        ...
        loss= torch.stack((l_pred_x,l_pred_y,l_pred_theta,l_pred_delta,l_x,l_y,l_theta,l_delta),dim=0).flatten()
        
        return loss
            
    def predict(self,x,y):
       
        l_pred_x = y[:,0]-torch.matmul(self.get_h(x),self.betas[0:self.nodes])
        l_pred_y = y[:,1]-torch.matmul(self.get_h(x),self.betas[self.nodes:2*self.nodes])
        l_pred_theta = y[:,2]-torch.matmul(self.get_h(x),self.betas[self.nodes*2:3*self.nodes])
        l_pred_delta = y[:,3]-torch.matmul(self.get_h(x),self.betas[self.nodes*3:4*self.nodes])
